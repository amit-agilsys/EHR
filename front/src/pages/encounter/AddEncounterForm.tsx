...
import { AsyncSearchFormField } from "./AsyncSearchFormField";
import PatientTypeSection from "./PatientTypeSection";
...
export default function AddEncounterForm({
  ...
  const { addEncounter, editEncounter } = useEncounter(
    false,
    GET_ALL_ENCOUNTERS
  );
  ...
  useEffect(() => {
    if (encounterToEdit && encounterToEdit.id) {
      ...
      // Set default options for async selects
      if (encounterToEdit.patientId && encounterToEdit.patientName) {
        setDefaultPatient({
          value: encounterToEdit.patientId,
          label: encounterToEdit.patientName,
        });
      }
  ...
  }, [addSuccess, editSuccess, onClose]);
      ...
      >
        <div className="grid grid-cols-3 gap-4 py-4">
          <AsyncSearchFormField
            ...
            apiEndpoint="/Patient/SearchPatientByName"
            defaultOption={defaultPatient}
          />
